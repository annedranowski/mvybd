...
$ \cO_s$ is the completion of the local ring $ \mathcal O_{\PP, s} $ and thus the formal spectrum of $ \cO_s$ is the formal neighbourhood $ D_s$ of $ s $ (also called the formal disc centered at $ s$).  Similarly the formal spectrum of the field $\cK_s$ is the deleted formal neighbourhood (or punctured disc), denoted $ D_s \setminus \{s\} $.
...
\acom{Add warning that we will also write $(t-s)^\mu$ for its image in $\GL_m(K)/\GL_m(R)$. We hope it will be evident?}
...
We will abuse notation and denote $L(R)$ simply $L_0$ in the hope that the ring $R$ will be understood from the context. 
...
\acom{For example, we will frequently make use of the lattice $L_\lambda$ corresponding to the point $t^\lambda\in\Gr$ given $\lambda\in P$. If $e_1,\dots,e_m$ denote the standard basis elements of $L_0(\cO)$ then 
$$ 
...
L(a) := L \otimes_{\CC[t]} \cO_a %\,. % The containment is not part of the definition so I comment it out \subset \cK_a^m\,.
$$
and if $ L(a) = \cO_a^m$ then $L$ is said to be trivial at $a$. 
...
Set $\AA = \AA^1$ \acom{and let $V_0$ denote the trivial vector bundle}. 
...
\begin{itemize}
    ...
    $$
    \Gr = \left\{ (P, \varphi) : \text{$V$ is a rank $m$ vector bundle on $ D_0 $, $\varphi : V \xrightarrow{\sim} V_0 $ on $ D_0 \setminus \{0\} $} \right\} \,. 
    $$
    ...
    $$
    \Gr_s = \left\{ (P, \varphi) : \text{$V$ is a rank $m$ vector bundle on $ D_s $, $\varphi : V \xrightarrow{\sim} V_0 $ on $ D_s \setminus \{s\} $} \right\}\,.
    $$
    ...
    $$
        \Grth = \left\{ (P, \varphi) : \text{$V$ is a rank $m$ vector bundle on $ \PP $, $\varphi : V \xrightarrow{\sim} V_0 $ on $ D_\infty $ } \right\}\,. % (a disc centered at $ \infty$)
    $$
...
Now fix effective dominant coweights $ \lambda_i,\mu_i \in P_{++}$ of size $N_i$ ($i=1,2$) and their sums $ \lambda = \lambda_1 + \lambda_2, \mu = \mu_1 + \mu_2$ of size $ N = N_1 + N_2$. 
...
\acom{At some point at least mention that we expect this to go through for permutations of $\mu_i\in P_{++}$ i.e.\ skew partitions? or non-dominant and non-negative coweights; so long as their sum $\mu_1 + \mu_2$ is effective dominant.}
...
\begin{itemize}
    ...
    \acom{What goes wrong if we take $G_1[t^{-1}]t^\mu$ instead?}
...
First we recall that for $ L \in \Gr^+$, $ L \in \Gr^{\lambda} = G(\cO)t^\lambda $ if and only if $ t |_{L_0/L} $ has Jordan type $ \lambda$.
    ...
    Now assume that $ (L, s) \in \Gr^{\lambda_1, \lambda_2}_{0,s}$. We know that \acom{by definition?} $ L(0) \in \Gr^{\lambda_1}$ and $L(s) \in \Gr^{\lambda_2} $.  This means that $t $ acting on 
    % $ \CC\xt^m / L(0)$ 
...
\begin{lemma} \label{le:Wmu}
    ...
    \begin{enumerate}
        \item $ L \in \cW_\mu$.
                \item $ L = \Sp_\cO(v_1, \dots, v_m)$ for some $ v_i $ of the form $ v_i = t^{\mu_i} e_i + \sum_{j=1}^m p_{ij} e_j $ where $ p_{ij} \in \CC[t] $ has degree less than $ \min(\mu_i, \mu_j)$. \acom{Do we want to sum over $j \ne i$ here? Maybe nvm because in $\TT_\mu$ the diagonal blocks can have $*$'s in the last row. Also $\cO$ sb $\CC[t]$.}
        \item  For all $ i $, 
...
\begin{proof}
    Let $ L \in \cW_\mu$.  Then $ L = \Sp_\cO(v_1, \dots, v_m) $ for some $ v_i $ with $ v_i = t^{\mu_i} e_i + \sum_{j=1} q_{ij}t^{\mu_i} e_j $ and $ q_{ij} \in t^{-1} \CC\xt[t^{-1}]$.  Since $ L \in \Grth^+ $, we see that $ v_i \in L_0$ which means that $ p_{ij} := q_{ij}t^{\mu_i} $ lies in $ \CC[t]$.  By construction, the polynomial $ p_{ij}$ has degree less than $ \mu_i$.
...
\begin{proof}
    Let $ L $ lie in this image.  Then $ L = g t^{\mu_1} (t-s)^{\mu_2}$ fom some $ g \in N(\CC[t, t^{-1}, (t-s)^{-1}]) $.  Let $ h =(t-s)^{\mu_2} t^{-\mu_2}  $.  Note that $ h \in T_1(\CC\xt[t^{-1}])$. 
    Moreover,
    $$ L = h (h^{-1} g h) t^{\mu}\,. $$
    Note that 
    % $ h^{-1} g h \in N(\CC\xT[t^{-1}])$ 
    $h^{-1} g h \in N(\cK_\infty)$ and from \cite{kamnitzer2014yangians} 
    \jcom{Check this}, we know that $ (h^{-1} g h) t^{\mu} \in \cW_\mu$ and so the result follows.

    \acom{
        % The argument from my thesis does not go through since $\cW_\mu$ is defined mod $\CC[t]$. More precisely, we still have $N(\Kinf) = N_1(\CC[t]) N(\CC\xt[t^{-1}])$ and so $h^{-1}g h=g_1g_2$ for some $g_1 \in N_1(\CC[t])$ for some $g_2\in N\xt[t^{-1}]$. But now we would like to write $g_2 t^\mu = t^\mu g_3$ for some $g_3\in\CC[t]$. Oh, but I guess we can also 
        To check this write $N(\Kinf) = N_1(\CC\xt[t^{-1}])N(\CC[t])$. Does it hold? If $\mu$ is dominant and $g\in N(\CC[t])$ then $gt^\mu = t^\mu g'$ for some $g'\in N(\CC[t])$.} % What is a textbook reference for these factorization claims? are they obvious; they seem obvi
    \end{proof}

\section{Climax}
Given $ A \in \TT_\mu$, recall the definition of $ g(A)$ given in section ??.  As $ g(A) \in M_n(\CC[t]) \cap G_1(\CC\xt[t^{-1}])$, we will regard $ g(A)$ as giving an element of $ \Grth^+ \cap \cW_\mu$.

The following result in \cite[Theorem 3.2]{cautis2018categorical}.
...
\begin{theorem} \label{th:OGrl}
    ...
    $$\overline{\OO}^{\lambda_1, \lambda_2}_\AA \cap \TT_\mu \cong \Gr^{\lambda_1, \lambda_2}_\AA \cap \cW_\mu $$
    given by $ (A,s) \mapsto (g(A), s)$.
\end{theorem}

\begin{proof}
Since we already have the isomorphism from Theorem \ref{th:TmuWmu}, it suffices to show that for any $ A \in \TT_\mu$, $$ (A,s) \in \overline{\OO}^{\lambda_1, \lambda_2}_\AA \cap \TT_\mu \text{ if and only if } (g(A), s) \in \Gr^{\lambda_1, \lambda_2}_\AA \cap \cW_\mu $$
...
\begin{proof}
 ...
 On the one hand, if $ (A,s) \UU^{\mu_1, \mu_2}_\AA $, then $ g(A)$ is upper triangular with diagonal $ t^{\mu_1} (t-s)^{\mu_2}$, and so $ g(A) \in N[t, t^{-1}, (t-s)^{-1}] t^{\mu_1} (t-s)^{\mu_2}$.
 
 On the other hand, if $ (g(A), s) \in S^{\mu_1, \mu_2}_\AA$, then we can write 
 $$
 g t^\mu r= n t^{\mu_1} (t-s)^{\mu_2}
$$
for some $ r \in G(\CC[t]), n \in N[t, t^{-1}, (t-s)^{-1}] $ and $ g = g(A)t^{-\mu}$.  Let $ h = (t-s)^{\mu_2} t^{-\mu_2}$ which lies in $ T_1 \CC[[t]]$.  Note that $ h^{-1}n h \in N((t^{-1}))$, so we can factor it as $ n = n_1 n_2 $, where $ n_1 \in N_1(\CC[[t^{-1}]]), n_2 \in N(\CC[t])$.  So then after doing a bit of algebra, we reach
$$
t^\mu r (t^{-\mu} n_2 t^\mu) t^{-\mu} = g^{-1} h n_1
$$
The right hand side $ g^{-1} g n_1 $ lies in$ G_1(\CC[[t^{-1}]]) $.  Now on the left hand side, we know that since $ \mu $ is dominant $ t^{-\mu} n_2 t^\mu \in N(\CC[t])$.  Moreover since $ \mu $ is dominant, we know that $$t^\mu G(\CC[[t]]) t^{-\mu} \cap G_1(\CC[[t^{-1}]]) = N_1(\CC[[t^{-1}]])$$
 Thus, we deduce that $ g^{-1} h n_1 \in N_1(\CC[[t^{-1}]])$ and hence $ g(A) \in N_1(\CC[[t^{-1}]]) t^{\mu_1} (t-s)^{\mu_2} $.  Given that we know that $ A \in \TT_\mu $, this implies that $ (A,s) \UU^{\mu_1, \mu_2}_\AA$ as desired.
\end{proof}
...
We view MV cycles as semi-standard young tableaux. Suppose we have two tableaux $\tau_1$ and $\tau_2$ with respective weights $\lambda_i$ and $\mu_i$ and we wished to taked their fusion product. We consider a generic matrix $X \in \UU^{\lambda_1,\lambda_2}_{0,s}$. 

The subtableaux of $\tau_1$ and $\tau_2$ where we take the first $j$ entries of each $\mu_i$ correspond to the submatrix $X_j$ of $X$ that consists of the first $j$ blocks. These subtableaux impose rank conditions, equivalently vanishing minors, on $X_j$ which allows us to deduce relations on the variables in $X$. The relations must also include the new variables in the latest block and cannot have $s$ as a factor.
...
$$\young(1123,23)$$
which corresponds to the MV cycle that is the cross product of the MV cycles corresponding to $\tau_1$ and $\tau_2$.
\end{example}
