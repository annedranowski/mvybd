...
\end{corollary}
The irreducible components of the closure of the intersection on the righthand side are \textbf{MV cycles of type $\lambda$ and weight $\mu$}. By BKK the set of MV cycles of all possible types and weights can be glued together to form a \textbf{perfect basis} of $\CC[N]$. 
...
In \cite{mvcycle}, the third author gave a combinatorial description of MV cycles for any reductive group, using Mirkovic-Vilonen polytopes, or equivalently Lusztig data.  For $ GL_m$, this description can be reworded in terms of Young tableaux. 
\jcom{Maybe add something here about Lusztig data and the standard reduced word.}
...
The Lusztig datum $n_\bullet(\tau)$ of the tableau $\tau$ is a list of $ m(m-1)/2$ non-negative integers defined from its \textbf{Gelfand--Tsetlin pattern} $\gt(\tau) = (\lambda(i)_j)_{1\le j\le i\le m}$ \acom{cite BZ} by the formula 
$$
...
% 
We can associate to $\tau$ an MV cycle. \acom{which is something we haven't introduced yet.} Let 
%  
\begin{equation*}
    Z(\tau) = 
     \overline{
...
The following result is closely related to Theorem 4 from \cite{dranowski2020generalized}.  It is also closely related to the description of MV cycles in terms of Kostant data obtained by Anderson-Kogan \cite{andersonkogan1} (see Section 9 in \cite{mvcycle}).  We also note that a different map from Young tableaux to MV cycles was obtained by Gaussent-Littelmann-Nguyen \cite[Theorem 2]{GLN}.
\begin{proposition}
    \label{pr:newmvdes}
    $Z(\tau)$ the MV cycle whose Lusztig datum with respect to $ \vi $ is $n_\bullet(\tau)$. 
\end{proposition}
\begin{proof}
...
\end{equation*}
From Corollary 9.7 of \cite{mvcycle}, we see that $ Z(\tau)_1$ is equal to $ t^\mu A(n_\bullet(\tau))^T$, where $ A(n_\bullet)$ is defined in section 4.3 of \cite{mvcycle}.  Thus, $ \overline{Z(\tau_1)}$ is the MV cycle whose Lusztig datum is $ n_\bullet(\tau) $.  We also note that $Z(\tau_1) \subset \Gr^\lambda$ is dense in $ Z(\tau_1)$.  
...
\begin{proof}
    ...
    L\cap \CC[t]^i = l. 
    $$
    By definition of $X'\ast_s X''$,  $A\big|_{\CC^{|\mu(i)|}}$ has Jordan type $((\lambda'(i),0),(\lambda''(i),s))$ and lies in $\UU^{\mu'(i),\mu''(i)}_{0,s}$. So, by \Cref{th:OTGrW}, $l\in\Gr_{0,s}^{\lambda'(i),\lambda''(i)} \cong \Gr^{\lambda'(i)}\times \Gr^{\lambda''(i)}$ so $(l(0),l(s))\in Z(\tau')\times Z(\tau'')$.
\end{proof}
...
What is a GOV? Let $X(\tau)$ be the top-dimensional \acom{speak to this?} irreducible component of
