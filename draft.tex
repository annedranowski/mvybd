...
% 
We define $ X_1 \ast_{\AA} X_2 $ to be the scheme-theoretic closure of $  \tau^{-1}(X_1 \times X_2) $ inside of $ \Grbd $.  By construction, this is a flat family over $ \AA$.  We write $X_1 \ast_s X_2 $ scheme-theoretic fibre over a point $ s \in \AA$.  
...
\acom{No I guess we haven't used it yet. Maybe in Proposition 5..}
...
\end{proposition}
% 
Denote by $Z(\tau)\subset\Gr$ the closure of the image of $X(\tau)$ under the isomorphism of \Cref{cor:mvy}. 
\acom{Still need Lusztig data of MV cycles; with it $B(\infty)$, stable vs ordinary, etc?}
\jcom{Do you want $ Z(\tau)$ to be the closure?}
\acom{Sure}
...
\end{proposition}
\acom{We would like to use the action of $W$ which gives isomorphisms
$Gr^\lambda \cap Gr_\mu \cap S^\mu \cong Gr^\lambda \cap Gr_{w\mu} \cap S^{w\mu}$ to generalize to non-dominant $\mu$. But the Lusztig datum of $\tau\in YT(\lambda)_\mu$ may not appear as the Lusztig datum of any element of $YT(\lambda)_{w\mu}$..}
% This gives us the dimension of Gr^\lambda \cap Gr_\mu \cap S^\mu.
\begin{lemma}
Alternate description. Works for non-dominant $\mu$. 
$$
Z(\tau) = \{
L\in\Gr : L\cap\CC[t]^i\in\Gr^{\lambda(i)}\cap S^{\mu(i)}\text{ for all }i \}
$$
is an MV cycle. 
\end{lemma}
...
\begin{proof}
    ...
    L\cap \CC[t]^i = l. 
    $$
    By definition of $X'\ast_s X''$,  $A\big|_{\CC^{|\mu(i)|}}$ has Jordan type $((\lambda'(i),0),(\lambda''(i),s))$ and lies in $\UU^{\mu'(i),\mu''(i)}_{0,s}$. So, by \Cref{th:OTGrW}, $l\in\Gr_{0,s}^{\lambda'(i),\lambda''(i)}\cap S^{\mu'(i),\mu''(i)}_{0,s}$ which fits the alternate description of $Z(\tau)$..
    
    \acom{..assuming $\tau_s$ identifies these intersections with products of intersections.}
    
    \jcom{I removed the $, s)$ from 1.  I don't understand the statement of 2.}
    \acom{Yup, me neither. Getting there..}
    ...
    \acom{Here $A$ is $\CC[t]$ and $M = \cO$ right? Better, agreed. Does the direction: torsion free implies flat, or not flat implies not torsion free, hold even without the PID assumption? This belongs way back in Joel's Lemma}
...
\acom{We compute zero fibres of $\ast_s$ by applying the results of \Cref{dthesis}..}
