...
\begin{itemize}
    ...
    $$
        \Grth = \left\{ (V, \varphi) : \text{$V$ is a rank $m$ vector bundle on $ \PP $, $\varphi : V \xrightarrow{\sim} V_0 $ on $ D_\infty $ } \right\}\,. % (a disc centered at $ \infty$)
    $$
...
for each $ s \ne 0$,  
\rcom{Joel: usually written $\Gr_{0,\AA^\times} \cong \Gr \times \Gr_s \times \AA^\times$, cite MVi} 
...
\begin{itemize}
    ...
    \acom{What goes wrong if we take $G_1[t^{-1}]t^\mu$ instead?} \jcom{Then it lives in the thin Grassmannian.  And unless $ A $ is nilpotent, then $g(A)$ won't live there.}
...
\begin{proof}
    ...
    \acom{
        ...
        \jcom{Yes, that is correct.  I think that I once had Alex write out a careful proof of this fact.  It holds for any unipotent group.  The proof is by induction.}
    \end{proof}
...
$$
The right hand side $ g^{-1} h n_1 $ lies in $ G_1(\CC\xt[t^{-1}]) $.  Now on the left hand side, we know that since $ \mu $ is dominant $ t^{-\mu} n_2 t^\mu \in N(\CC[t])$.  
