...
\end{definition}  
    ...
    % 
     ...
     Its closure is $ \overline{\OO}^{\lambda', \lambda''}_{0,s} = \bigcup_{\gamma' \le \lambda', \gamma'' \le \lambda''} \OO^{\gamma', \gamma''}_{0,s}$.
     ...
     \#\text{~boxes in first $i$ columns of }\lambda' \quad
...
\jcom{I don't know if it is really necessary to introduce this $ k$.  Later it will be the number of columns of $ \lambda$, but we could just regard $ \lambda$ to have some columns of size 0.}
...
Let $ z \in \AA^k$. Some of the coordinates of $z$ are equal; we introduce some data to describe these equalities.  There exist unique $ 0=k_0< k_1< \dots < k_r = k $, distinct $ s_1, \dots, s_r \in \CC$ and a minimal length permutation  $ p $ of $ \{1, \dots, k\}$ such that $$ s_j = z_{p(k_{j-1}+1)} = \cdots = z_{p(k_j)} $$ 
...
For $ s \ne 0$, we see that the equalities in $ z(s) $ reproduces the permutation $ p$, that $ r = 2 $, and that
...
r_{z(s), 1}(i) =  N - 
     ...
     \end{equation}
On the other hand, for $z(0)$, we get $ r = 1$ and that 
...
Let $ W = \{ z(s) : s \in \CC \}$.  Combining (\ref{eq:ranks}), (\ref{eq:ESfibre}), (\ref{eq:rcols}), (\ref{eq:rcols2}) we see that the Eisenbud-Saltman family $ X_{r, W} \rightarrow W $ gives our family $ \overline{\OO}^{\lambda', \lambda''}_{0, \AA}$.
\end{proof}
...
\subsection{The Mirkovic-Vybornov slice} Fix the ``$\mu$-numeration'' \((e^1_1,\ldots,e^{\mu_1}_1,\ldots,e^1_m,\ldots,e^{\mu_m}_m)\) of the standard basis of $\CC^N$. 
...
% \acom{Change $L_0$}
    ...
    $\CC[t]^m/L\to \cO^m/L(0)$ induces an isomorphism between the $0$-generalized eigenspace of $ t$ and $ \cO^m / L(0)$.   The same thing holds for the $s$-generalized eigenspace of $t $ and $ \cO_s^m/L(s)$. This shows that (i) implies (ii) and the logic can be reversed to see that (ii) implies (i). \acom{is this map $v + L \mapsto v + L(0)$?} \jcom{yes} \marginpar[]{*}
    % todo 
