...
\acom{For example, we will frequently make use of the lattice $L_\lambda$ corresponding to the point $t^\lambda\in\Gr$ --- not yet introduced --- given $\lambda\in P$. If $e_1,\dots,e_m$ denote the standard basis elements of 
% $L_0(\cO)$ then 
$\cO^m$ then 
$$ 
...
\begin{enumerate}
    ...
    % 
    Lastly, in group-theoretic terms, this isomorphism is given using the diagonal map
    $$
    G(\CC[t, t^{-1}, (t-s)^{-1}])/G(\CC[t]) \rightarrow G(\cK)/G(\cO) \times G(\cK_s)/G(\cO_s)\,,
    $$
    % This
    which is compatible with the lattice description, since for $ g \in G(\CC[t, t^{-1}, (t-s)^{-1}])/G(\CC[t]) $, we have
    $$
...
Now fix effective dominant coweights 
% $\lambda_1, \lambda_2 \in P_{++}$ 
$\lambda',\lambda''\in P_{++}$ of sizes 
% $N_1, N_2$  
$N',N''$ respectively, and let $ \lambda = \lambda' + \lambda''$ whose size $N' + N''$ is denoted $N$. \acom{This $N$ conflicts with $N\subset G$ the unipotent subgroup.}
% $ N = N_1 + N_2$. 
...
\begin{itemize}
    ...
    % 
    \item The family $ \overline{\Gr}_{0,\AA}^{\lambda',\lambda''} \rightarrow \AA$ defined by $ \overline{\Gr}^{\lambda', \lambda''}_{0, \AA} = \overline{\Gr}^{\lambda'} \ast_\AA \overline{\Gr}^{\lambda''}$. 
    \item \acom{NOTE TO SELF CONTINUE HERE}
...
\begin{itemize}
    ...
    % 
    \item The family of adjoint orbits $ \overline{\OO}^{\lambda_1, \lambda_2}_{0,\AA} \rightarrow \AA$ whose fibre over $ s \ne 0 $ is $ \overline{\OO}^{\lambda_1, \lambda_2}_{0,s}$ and whose fibre over 0 is $\overline{\OO}^\lambda$ a consequence of Eisenbud and Salten's description/characterization(?) of ``rank varieties'' as proved in \Cref{cor:es}.
