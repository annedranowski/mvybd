...
Set $\AA = \AA^1$ and let $V_0$ denote the trivial rank $ m $ vector bundle. \acom{Change $V_0$} 
...
\begin{itemize}
    ...
    $$
    We obtain the lattice from $ (V,\varphi) $ by setting $ L = \Gamma(D_0, V)$ which is embedded into $ \cK^m = \Gamma(D_0^\times, V_0)$ using $ \varphi$.  On the other hand, we obtain the lattice from the group-theoretic description $ G(\cK) / G(\cO) $ by setting $ L = g \cO^m$ for $ g \in G(\cK)$.\emoji{clap} \acom{Change $V_0$}
    % 
    \item For any $ s \in \CC $, 
    % \acom{should it be $\AA$ instead of $\CC$} 
    the ordinary affine Grassmannian $\Gr_s = G(\cK_s)/G(\cO_s)$ at $ s $. It is described in modular terms by \acom{Change $V_0$}
    $$
    % \Gr_s = 
...
\begin{lemma} \label{le:Grl1l2}
    ...
    \begin{enumerate}
        ...
        \item The linear operator $ t $ on $ L_0/L$ has Jordan type $(0,\lambda_1), (s,\lambda_2)$. \acom{vestigial $L_0$ change to $\CC[t]^m$}
...
\begin{proof}
    Let $ L \in \cW_\mu$.  Then $ L = \Sp_{\CC[t]}(v_1, \dots, v_m) $ for some $ v_i $ with $ v_i = t^{\mu_i} e_i + \sum_{j=1} q_{ij}t^{\mu_i} e_j $ and $ q_{ij} \in t^{-1} \Oinf$.  Since $ L \in \Grth^+ $, we see that $ v_i \in L_0$ which means that $ p_{ij} := q_{ij}t^{\mu_i} $ lies in $ \CC[t]$.  By construction, the polynomial $ p_{ij}$ has degree less than $ \mu_i$. \acom{Change $L_0$}
