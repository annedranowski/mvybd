...
\begin{itemize}
    ...
    $$
    % \Gr = 
    \left\{ (V, \varphi) : \text{$V$ is a rank $m$ vector bundle on $ D_0 $, $\varphi : V \xrightarrow{\sim} V_0 $ on $ D_0^\times $} \right\} \,. 
    $$
    ...
    $$
    We obtain the lattice from $ (V,\varphi) $ by setting $ L = \Gamma(D_0, V)$ which is embedded into $ \cK^m = \Gamma(D_0^\times, V_0)$ using $ \varphi$.  On the other hand, we obtain the lattice from the group-theoretic description $ G(\cK) / G(\cO) $ by setting $ L = g \cO^m$ for $ g \in G(\cK)$.\emoji{clap}
    % 
    ...
    $$
    % \Gr_s = 
    \left\{ (V, \varphi) : \text{$V$ is a rank $m$ vector bundle on $ D_s $, $\varphi : V \xrightarrow{\sim} V_0 $ on $ D^\times_s $} \right\}\,.
    $$
    ...
    $$
        % \Grth = 
        % \left\{ (V, \varphi) : \text{$V$ is a rank $m$ vector bundle on $ \PP $, $\varphi : V \xrightarrow{\sim} V_0 $ on $ D_\infty $ } \right\}\,. % (a disc centered at $ \infty$)
    $$
...
\begin{itemize}
    ...
    Over $s=0$ the fibre is shown to be $ \overline{\Gr}^{\lambda_1 + \lambda_2}$ in the proof of \cite[Proposition 3.1.14]{zhu2016introduction}. 
    ...
    % 
    \acom{What goes wrong if we take $G_1[t^{-1}]t^\mu$ instead?} \jcom{Then it lives in the thin Grassmannian.  And unless $ A $ is nilpotent, then $g(A)$ won't live there.} \acom{But our $A$ is always nilpotent? Another notational thing is $G_1\xt[t^{-1}] = G_1(\CC\xt[t^{-1}])$ and in our paper this short hand is only used for these kernel subgroups.}
    ...
    $$
    % S^{\mu_1, \mu_2}_{0,s} = 
    N(\CC[t, t^{-1}, (t-s)^{-1}])t^{\mu_1} (t-s)^{\mu_2} G(\CC[t])\subset G(\CC[t,t^{-1}, (t-s)^{-1}]) / G(\CC[t]) \,. % \G(\CC[t])
    $$
    % TODO: resolve
...
\begin{lemma} \label{le:Grl1l2}
    ...
    \begin{enumerate}
        ...
        \item The linear operator $ t $ on $ L_0/L$ has Jordan type $(0,\lambda_1), (s,\lambda_2)$. \acom{vestigial $L_0$}
...
\begin{lemma} \label{le:Wmu}
    ...
    \begin{enumerate}
        \item $ L \in \cW_\mu$.
                \item $ L = \Sp_{\CC[t]}(v_1, \dots, v_m)$ for some $ v_i $ of the form $ v_i = t^{\mu_i} e_i + \sum_{j=1}^m p_{ij} e_j $ where $ p_{ij} \in \CC[t] $ has degree less than $ \min(\mu_i, \mu_j)$. \acom{Do we want to sum over $j \ne i$ here? Maybe nvm because in $\TT_\mu$ the diagonal blocks can have $*$'s in the last row. Also $\cO$ should be $\CC[t]$.}
        \item  For all $ i $, 
...
\begin{proof}
    Let $ L \in \cW_\mu$.  Then $ L = \Sp_{\CC[t]}(v_1, \dots, v_m) $ for some $ v_i $ with $ v_i = t^{\mu_i} e_i + \sum_{j=1} q_{ij}t^{\mu_i} e_j $ and $ q_{ij} \in t^{-1} \Oinf$.  Since $ L \in \Grth^+ $, we see that $ v_i \in L_0$ which means that $ p_{ij} := q_{ij}t^{\mu_i} $ lies in $ \CC[t]$.  By construction, the polynomial $ p_{ij}$ has degree less than $ \mu_i$.
...
\end{lemma}
\acom{the map $ \Grbd \rightarrow \Grth$ caused some confusion in my talk as on the left we have a family and on the right we have the thick affine Grassmannian --- Jiuzu kept thinking aloud that we can make $\Grth$ a family too.. Anyway I think that you address this below Joel. Probably it is worth writing out the map just before this first occurence? \textit{is it $(L,s)\mapsto L$?}}
...
\begin{proof}
 ...
 On the one hand, if $ (A,s) \in \UU^{\mu_1, \mu_2}_{0,\AA} $, then $ g(A)$ is upper triangular with diagonal $ t^{\mu_1} (t-s)^{\mu_2}$, and so $ g(A) \in N[t, t^{-1}, (t-s)^{-1}] t^{\mu_1} (t-s)^{\mu_2}$.
...
and hence $ g(A) \in N_1(\Oinf) t^{\mu_1} (t-s)^{\mu_2} $.  Given that we know that $ A \in \TT_\mu $, this implies that $ (A,s) \in \UU^{\mu_1, \mu_2}_{0,\AA}$ as desired.
\end{proof}
