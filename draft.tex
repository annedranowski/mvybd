...
\acom{can we conflate or should we be precise and say coweight?}
...
$$ \Gr_s = \{ L \subset \cK_s^m : \text{ $L$ is a $\cO_s$-lattice} \} $$   
    ...
    $$
    where $D_\infty$ denotes a disk centered at $ \infty$. \acom{What is the difference between ``a disk'' and ``the formal disk''}
    ...
    \jcom{Maybe write $\Kinf$ for $ \CC\xT[t^{-1}] $?  Also maybe change the ``thick'' notation''.} \acom{Since $\sf th$ is also the first two letters of ``thin''?}
    ...
    In lattice terms, $ \Gr^+$ (resp. ${\Grth}^+$) contains those lattices $L$ which are contained in the standard \acom{previously called ``trivial''} lattice $ L_0(\cO)$ (resp. $L_0(\CC[t])$. % , i.e. $ L \subseteq L_0$.
\end{itemize}
...
\end{cases}
$$
In the $s\ne 0$ case this isomorphism is given in the lattice realization \acom{vs ``model''?} by taking trivializations; 
$$
L \mapsto (L(0), L(s))\,.
$$
% in the $ s \ne 0 $ case.
...
Via \ul{this} \acom{rather, the above? $L\mapsto (L(0),L(s))$} isomorphism $ \Gr_{0,s} \cong \Gr \times \Gr_s $,  for each $ s \ne 0$, we obtain a subscheme $ X_1 \times X_2 \times \AA^\times $  of $\Grbd$.  
...
\begin{itemize}
    ...
    \acom{do we prefer $\overline{\Gr}^\lambda$ to $\overline{\Gr^\lambda}$? I am for the former! Though it is less accurate??}
    This orbit lies inside the positive part $ \Gr^+$ of the affine Grassmannian.
    \item The family $ \overline{\Gr}^{\lambda_1, \lambda_2} \rightarrow \AA$, defined by $ \overline{\Gr}^{\lambda_1, \lambda_2} = \overline{\Gr}^{\lambda_1} * \overline{\Gr}^{\lambda_2}$. \acom{I guess that it is good to have the notation here, in place of $\overline{\overline\Gr^{\lambda_1}\times\overline\Gr^{\lambda_2}\times\AA^\times}$..}
    ...
    \item The family of semi-infinite orbits $ S^{\mu_1, \mu_2}_\AA := S^{\mu_1} * S^{\mu_2}$.  If $ s \ne 0 $, then the fibre $S^{\mu_1, \mu_2}_{0,s} $ is given by
    $$
