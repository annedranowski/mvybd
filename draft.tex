...
\jcom{I don't know if it is really necessary to introduce this $ k$.  Later it will be the number of columns of $ \lambda$, but we could just regard $ \lambda$ to have some columns of size 0.} \acom{And make it $N$? Was $k = m$ replaced because $m$ is already used?} \jcom{Yes and yes}
...
\begin{proposition}(\cite{baumann2019mirkovic})
    ...
    \begin{equation}
        ...
        \right) [Z(\tau)]
    \end{equation}
    and the intersection multiplicities on the right are found by the algorithm described in \Cref{ss:algo}. 
...
% 
Recall that there is an order on repeated entries of a semistandard Young tableau. 
% 
...
Let $m,\lambda',\lambda'',\lambda,\mu',\mu'',\mu,N$ be as above, and recall the $\mu$-numeration of the standard basis of $\CC^N$. Given $A \in M_N(\CC)$ we'll denote by $A\big|_{\CC^i}$ the restriction of $A$ to the subspace spanned by the first $i$ standard basis vectors. Given $(\tau',\tau'')\in YT(\lambda')_{\mu'}\times YT(\lambda'')_{\mu''}$ denote by $\lambda'(i),\lambda''(i)$ the shape of $\tau'(i),\tau''(i)$ resp.\ where $\tau^?(i)$ denotes the tableau got from $\tau^?$ by discarding all but its first $i$ boxes. 

\acom{May want a more compact notation such as $A_{k}$}

\begin{proposition}
    Given $Z',Z''$, let $(\tau',\tau'')\in YT(\lambda')_{\mu'}\times YT(\lambda'')_{\mu''}$ be such that\dots If $s\ne 0$, then 
    \[
    \overline{\{A\in\UU_{0,s}^{\mu',\mu''} : A \big|_{\CC^{\mu_1 + \cdots + \mu_i}} \in \OO_{0,s}^{\lambda'(i),\lambda''(i)} \text{ all } i=1,\dots,m\}} 
    \cong Z'\times Z'' \,. 
    % = Z(\tau')\times Z(\tau'')\,. 
    \]
\end{proposition}
...
\begin{proof}
    % \acom{Another notational possibility is $\mu = \mu^0 + \mu^s$ etc., or $\mu(0) + \mu(s)$ or some variant with the eigenvalues being the indices.}
    % 
    First, reformulate as 
    \[
    \begin{tikzcd}
        & (L,s) \in \Grbd \ar[dr,mapsto]\ar[dl,mapsto]\\
        t\big|_{\CC[t]^m/L} \in A & & (L(0),L(s)) \in B 
    \end{tikzcd}
    \]
    Fix $i \in \{1,\dots,m\}$, let $p = \mu_1 + \cdots + \mu_i$ and consider $(L',L'')\in Z'\times Z''$. 
    We need to show that the Jordan type of $t\big|_{\CC[t]^p/L(0)\cap\CC[t]^p}$ is $\lambda'{(p)}$ and Jordan type of $(t-s)\big|_{\CC[t]^p/L(s)\cap \CC[t]^p}$ is $\lambda''{(p)}$. 
...
Let us redo example 3 but with non-dominant weights $\mu'$ and $\mu''$ such that $\mu' + \mu''$ is effective dominant. Consider the tableaux
...
\begin{array}{c|c}
    ...
    X_4 & \begin{array}{c}
         ...
         A_{13}^2A_{24}^2A_{34}^1 - A_{13}^2A_{24}^1A_{34}^2 - A_{14}^1A_{24}^1 - sA_{14}^2A_{24}^1, A_{12}^3A_{23}^2A_{14}^1 - A_{12}^2A_{23}^2A_{14}^2 - A_{12}^3A_{13}^2A_{24}^1 + A_{12}^2A_{13}^2A_{24}^2,  \\
         A_{12}^3A_{13}^2A_{24}^1A_{34}^2 - A_{12}^2A_{13}^2A_{24}^2A_{34}^2 + A_{12}^3A_{14}^1A_{24}^1 - A_{12}^2A_{14}^2A_{24}^1
    \end{array} 
\end{array}.
\]
Our ideal decomposes into 
\[
\langle A_{12}^1, A_{12}^2, A_{13}^1, A_{23}^1, A_{23}^2, A_{24}^1, A_{34}^1,s \rangle
\cap \langle A_{12}^1, A_{12}^2, A_{13}^1, A_{23}^1, A_{34}^1, A_{23}^2A_{34}^2 + A_{24}^1, A_{13}^2A_{34}^2 + A_{14}^1, A_{13}^2A_{24}^1 - A_{23}^2A_{14}^1, s \rangle.
\]
The tableau for each ideal is
\[\begin{array}{cccc}\vspace{1mm}
    ...
    \langle A_{12}^1, A_{12}^2, A_{13}^1, A_{23}^1, A_{34}^1, A_{23}^2A_{34}^2 + A_{24}^1, A_{13}^2A_{34}^2 + A_{14}^1, A_{13}^2A_{24}^1 - A_{23}^2A_{14}^1, s \rangle &\leadsto \young(11123,224,34) &\equiv \young(123,24,4) &\leadsto Z_1 Z_3 Z_{P_2}
\end{array}
\]
