...
\acom{Add: Warning: We will also view $(t-s)^\mu$ in $\GL_m(K)/\GL_m(R)$. We hope it will be expedient and evident.}
...
\begin{itemize}
    ...
    In lattice terms, $ \Gr^+$ (resp. ${\Grth}^+$) contains those lattices $L$ which are contained in the \acom{add (relevant)?} standard lattice. 
...
\begin{enumerate}
    ...
    which is given in modular terms by restricting the trivialization to $ D_\infty$.  It is given in group-theoretic terms on the fibre over $ s $ by \acom{...inclusion?}
    $$
...
\acom{At some point at least mention that we expect this to go through for permutations of $\mu_i\in P_{++}$ i.e.\ skew partitions? or non-dominant, but also non-negative weights, so long as $\mu_1 + \mu_2$ is in $P_{++}$}
% 
\begin{itemize}
