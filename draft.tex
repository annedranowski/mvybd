...
\begin{itemize}
    ...
    \acom{What goes wrong if we take $G_1[t^{-1}]t^\mu$ instead?} \jcom{Then it lives in the thin Grassmannian.  And unless $ A $ is nilpotent, then $g(A)$ won't live there.} \acom{Another notational thing is $G_1\xt[t^{-1}] = G_1(\CC\xt[t^{-1}])$ and in our paper this short hand is only used for these kernel subgroups.}
...
\begin{proof}
    ...
    \acom{
        ...
        To check this write $N(\Kinf) = N_1(\Oinf)N(\CC[t])$. Does it hold? If $\mu$ is dominant and $g\in N(\CC[t])$ then $gt^\mu = t^\mu g'$ for some $g'\in N(\CC[t])$.} % What is a textbook reference for these factorization claims? are they obvious; they seem obvi
