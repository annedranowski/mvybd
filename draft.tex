...
\begin{itemize}
    \item The ordinary affine Grassmannian $\Gr = G(\cK)/G(\cO)$. It is described in modular terms by \acom{Where $V_0$ is the trivial rank $n$ vector bundle on $\PP$?}
    $$
...
$$   
    ...
    $$
        \Gr^{\sf th} = \{ (P, \varphi) : \text{$V$ is a rank $n$ vector bundle on $ \PP $, $\varphi : V \rightarrow V_0 $ on $ D_\infty $ } \} % (a disk centered at $ \infty$)
    $$
...
$$   
\jcom{Maybe write $\cK_\infty$ for $ \CC\xT[t^{-1}] $?  Also maybe change the ``thick'' notation''.}
...
Now fix $\mu_i\le\lambda_i$ of size $N_i$ ($i=1,2$) in $P$ and their sums $\mu = \sum \mu_i \le \lambda = \sum\lambda_i$ of size $N = \sum N_i$. 
% 
\begin{itemize}
    % \item Weights $\mu_i\le\lambda_i$ of size $N_i$ ($i=1,2$) 
    % \item Their sums $\mu = \sum \mu_i \le \lambda = \sum\lambda_i$ of size $N = \sum N_i$
    \item The point $L_\lambda = t^\lambda G(\cO)$ in $\Gr$ and the point ?? $t^\lambda G(\CC[t])$ in $\Gr^{\sf th}$ \acom{abandon lattice of coweight notation?}
    \item The orbit $\Gr^\lambda = G(\cO) L_\lambda$ \acom{and its closure?}
    \item The family $\Gr_\AA^{\lambda_1,\lambda_2}\to\AA$ whose generic fibre is $\Gr^{\lambda_1} \times \Gr^{\lambda_2}$ and whose zero fibre is $\Gr^{\lambda}$ \acom{1. and closures? 2. can probably be described as an orbit of some group scheme} 
