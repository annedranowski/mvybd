...
In \cite{kamnitzer2010mirkovic}, the third author gave a combinatorial description of MV cycles for any reductive group, using Mirkovic-Vilonen polytopes, or equivalently Lusztig data.  For $\GL_m$, this description can be reworded in terms of Young tableaux. 
...
$$
for $(i,j) = (1,1), (2,1), \dots, (m-1,1), (2,2),\dots,(m-1,2),\dots,(m-1,m-1)$. 
The pattern is recorded as a lower-triangular matrix (array of shapes of subtableaux) and the datum is recorded as a sequence. See \cite[\S 4]{berenstein1988tensor}.

\jcom{Something is wrong with the indexing of the Lusztig data, there should only be $ m(m-1)/2 $ entries.}

\acom{Yes $i = m$ doesn't make sense. Should be $i = 1,\dots,m-1$ and $j = 1\dots i$ or $(1,1),(2,1),..,(m-1,1),(2,2),..,(m-1,2),..,(m-1,m-1)$ which is $(m-1) + (m-1-1) + \cdots + 1$ things. Right?}
...
The following result is closely related to Theorem 4 from \cite{dranowski2020generalized}.  It is also closely related to the description of MV cycles in terms of Kostant data obtained by Anderson--Kogan \cite{anderson2006algebra} \acom{is this the right AK?} (see Section 9 in \cite{kamnitzer2010mirkovic}).  We also note that a different map from Young tableaux to MV cycles was obtained by Gaussent--Littelmann--Nguyen \cite[Theorem 2]{gaussent2013knuth}.
\begin{proposition}
    \label{pr:newmvdes}
...
\end{equation*}
From \cite[Corollary 9.7]{kamnitzer2010mirkovic}, we see that $ Z(\tau)_1$ is equal to $ t^\mu A(n_\bullet(\tau))^T$, where $ A(n_\bullet)$ is defined in section 4.3 of \cite{kamnitzer2010mirkovic}.  Thus, $ \overline{Z(\tau_1)}$ is the MV cycle whose Lusztig datum is $ n_\bullet(\tau) $.  We also note that $Z(\tau_1) \subset \Gr^\lambda$ is dense in $ Z(\tau_1)$.  
\jcom{Need to justify this last sentence.} \acom{We mean $Z(\tau_1)\cap \Gr^\lambda$ is dense in $Z(\tau_1)$?}
...
% 
% 
In analogy with the BD construction \acom{add loc cit} we define $X(\tau')\ast_0 X(\tau'')$ to be the zero fibre of the family $\overline{\bigcup_{s\in\CC\setminus\{0\}} X(\tau')\ast_s X(\tau'')}$ in $M_N(\CC)$. 
% 
\begin{corollary}
