...
We will also study the affine space of $m$x$m$ matrices which we denote $ M_m$.
...
$$ \Gr_s = \{ L \subset \cK_s^m : \text{ $L$ is a $\cO_s$-lattice} \} $$   
    ...
    $$
    where $D_\infty$ denotes a disk centered at $ \infty$. \acom{What is the difference between ``a disk'' and ``the formal disk''} \jcom{No difference.}
    ...
    \jcom{Maybe write $\Kinf$ for $ \CC\xT[t^{-1}] $?  Also maybe change the ``thick'' notation''.} \acom{Since $\sf th$ is also the first two letters of ``thin''?} \jcom{That's a good point!  Maybe we could use some kind of bold  Gr (``thick'' letters) or maybe that would be too confusing.}
    ...
    In lattice terms, $ \Gr^+$ (resp. ${\Grth}^+$) contains those lattices $L$ which are contained in the standard \acom{previously called ``trivial''} \jcom{I changed it so that $ R^m $ is called the standard lattice.  But I would like to leave the terminology ``trivial at $ a$'' is that ok, or should we write ``standard at $a$''.} lattice $ L_0(\cO)$ (resp. $L_0(\CC[t])$. % , i.e. $ L \subseteq L_0$.
\end{itemize}
...
\end{cases}
$$
In the $s\ne 0$ case, the isomorphism is given in the modular realization by restricting the vector bundle and trivialization to the appropriate disks.  In the lattice realization, it is given by forming tensor products 
$$
L \mapsto (L(0), L(s))\,.
$$
% in the $ s \ne 0 $ case.
...
\jcom{I think that we will need notations for both things, but I'm happy to use $ \ast$ for the central fibre and use something else for the whole family.}
...
\begin{itemize}
    ...
    \item The Mirkovic-Vybnornov slice $\TT_\mu$.  \jcom{Anne, can you add this definition?}
