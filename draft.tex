...
\begin{definition} 
\label{def:grplus}
The \textbf{positive parts} of $\Gr $ (resp.\ $\Grth$),  is defined by 
\begin{gather*}
...
Let $ \Gr_{0, \CC^\times} $ denote the preimage of $\AA \setminus \{0\}$ under the map $\Gr_{0,\AA} \rightarrow \AA $.  Composing the isomorphisms relating $\Grbd$ to $\Gr_s$ and $\Gr_s$ to $\Gr$, (\cref{it:polyno-taylor,it:grs-gr} of \cref{ss:relsbtwgrs} preceding), we obtain an isomorphism $ \Gr_{0,s} \cong \Gr \times \Gr$.
...
% Suppose that $ s \ne 0$.  
Moreover, if $s\ne0$, then in the fibre $\overline{\Gr}_{0,s}^{\lambda', \lambda''}$ we have the open locus $ \Gr_{0,s}^{\lambda', \lambda''}$ coming from transporting $ \Gr^{\lambda'} \times \Gr^{\lambda''} $ via the isomorphism $ \Gr_{0,s} \cong \Gr \times \Gr_s $. \acom{reference \cref{it:grs-gr}?} This open locus is a $ G(\CC[t])$-orbit
...
For $ s \ne 0$, we see that the equalities in $ z(s) $ give rise to the data of $ r = 2$, $k_1$, $s_1 = 0$, $s_2 = s$, and the permutation $p $.
We also see that
\begin{equation} 
\label{eq:rcols}
    \begin{split}
...
\begin{theorem}
\label{th:OTGrW}
The isomorphism from \Cref{th:OGrl} \acom{should ``Theorem'' etc.\ be capitalized when it appears in the middle of a sentence?}
...
Let $ A \in \TT_\mu$ and $ s \in \CC $. We must show that  $ (A,s) \in \UU^{\mu', \mu''}_{0,\AA} $ if and only if $ (g(A),s) \in S^{\mu', \mu''}_{0,\AA} $. \acom{not $g(A)\CC[t]^m$}

On the one hand, if $ (A,s) \in \UU^{\mu', \mu''}_{0,\AA} $, then $ g(A)$ is lower-triangular with diagonal $ t^{\mu'} (t-s)^{\mu''}$, and so $ g(A) \in N_-[t, t^{-1}, (t-s)^{-1}] t^{\mu'} (t-s)^{\mu''}$. \acom{not $g(A)\CC[t]^m$}

On the other hand, if $ (g(A), s) \in S^{\mu', \mu''}_{0,\AA}$, \acom{not $g(A)\CC[t]^m$} then we can write 
$$
...
% 
Restricting to the zero fibre we recover \cite[a main result]{dthesis}, itself a corollary of the ordinary affine Grassmannian version of \Cref{th:OGrl}, \cite[Theorem~4.4.1(c)]{mirkovic2019comparison}. \acom{Technically the zero fibre is comprised of $\CC[t]$ lattices and we need to `specialize' to end up in the ordinary $\Gr$.}
% 
\begin{corollary}
    \label{cor:mvy}
    $\OO^\lambda \cap \TT_\mu\cap\n \cong\overline\Gr^\lambda\cap S^\mu$. 
\end{corollary}
...
\acom{Cite Joel? Hide dependence on reduced expression?}
The Lusztig datum $n_\bullet(\tau)$ of the tableau $\tau$ is got from its \textbf{Gelfand--Tsetlin pattern} $\gt(\tau) = (\lambda(i)_j)_{1\le j\le i\le m}$ \acom{cite BZ} by the formula 
$$
...
% 
% TODO
% \begin{theorem}(CITE KAM07)
%     Taking $\vi$-Luszig datum is a bijection $P\to\NN^{\ell}$. 
% \end{theorem}
% 
...
\begin{proposition}
    ...
    The decomposition
    \begin{equation*}
        % \label{eq:govsasirrecs}
        \overline{\OO}^\lambda\cap\TT_\mu\cap\n = \bigsqcup_{\tau\in YT(\lambda)_\mu} X(\tau)
    \end{equation*}
    is\dots
\end{proposition}
% 
Denote by $Z(\tau)\subset\Gr$ the image of $X(\tau)$ under the isomorphism of \Cref{cor:mvy}. 
\acom{Still need Lusztig data of MV cycles; with it $B(\infty)$, stable vs ordinary, etc?}
% 
\begin{proposition}
    \label{prop:dth}
    % The isomorphism 
    % $\phi: \overline{\OO}^\lambda\cap\TT_\mu\cap\n \to \overline\Gr^\lambda\cap S^\mu_-$ 
    % is compatible with the decomposition of \Cref{pr:govsasirrecs}
    % the bijection $YT(\lambda)_\mu \to \irr \overline{\OO}^\lambda\cap\TT_\mu\cap\n$ 
    % in the sense that 
    $Z(\tau)$ is dense in the MV cycle whose Lusztig datum is $n_\bullet(\tau)$. 
\end{proposition}
...
% 
% 
In this part we prove that the product 
% the ideal of 
% $Z'\ast_\AA Z''$ 
$b_{Z'}b_{Z''}\in\CC[N]$ can be computed from a ``minimal'' pair $(\tau',\tau'')$ of semistandard Young tableaux for the Lusztig data  $(n_\bullet(Z'),n_\bullet(Z''))$. 
% 
To begin, let $\tau',\tau''$ be given and let $m,\lambda',\lambda'',\lambda,\mu',\mu'',\mu,N$ be as before.
% 

\acom{We may want to introduce $\ast_s$ in general. For now we make a definition.} Given $s\in\CC$, let 
\begin{equation*}
    X(\tau')\ast_s X(\tau'') = \left\{A\in\UU_{0,s}^{\mu',\mu''} : A \big|_{\CC^{|\mu(i)|}} \in \OO_{0,s}^{\lambda'(i),\lambda''(i)} \text{ all } i=1,\dots,N\right\}\,,
\end{equation*}
% 
and let $Z'\ast_s Z'' = (Z'\ast_\AA Z'')_s$. If $s\ne 0$ identify $Z'\ast_s Z''= Z'\times Z''$ via $\Gr_{0,s}\cong \Gr\times\Gr$ \acom{or make the identification using the map the map $\tau$ of \Cref{eq:mvitau}. Can similarly identify $X'\ast_s X'' = X'\times X''$ by performing the obvious change of basis and block-diagonalizing. May make sense to introduce $\ast_s$ in greater generality, earlier; or at least on MV cycles earlier, since here I should actually make the definition for $Z(\tau)$'s.. which is ok, as in not a loss of generality. Still..}
\begin{proposition}
...
\begin{proof}
    ...
    % \[
    % \begin{tikzcd}
    %     \TT_{\mu(i)} \ar[r] \ar[d] & B \ar[d] \\ 
    %     \TT_{\mu(i-1)} \ar[r] & ?? 
    % \end{tikzcd}
    % \]
    % commutes.
    Let $i = 1,\dots, m$, let $a = 0,s$, and denote by $E_a$ the generalized $a$-eigenspace of $A$. 
    Then 
    \begin{enumerate}
        \item $(g(A)\CC[t]^m\cap\CC[t]^i,s) = g(A\big|_{\CC^{|\mu(i)|}})\CC[t]^i$
        \item \label{eq:twoingredients} $g((A-a)\big|_{E_a \cap \CC^{|\mu(i)|}},a) = \left(g(A) \CC[t]^m \otimes_{\CC[t]}\cO_a \right)\cap \cO_a^i$
        % g(A\big|_{\CC^{|\mu(i)|}})\CC[t]^i \otimes_{\CC[t]} \cO_a $
    \end{enumerate}
    \Cref{eq:twoingredients} follows requries \Cref{le:linalg} and the fact that a module over a PID is flat if and only if it is torsion free. % and $\cO_a$ is clearly torsion free
    ...
    % \[
    % Z(\tau) = \{L \in \Gr: L\cap\CC[t]^i \in \Gr^{\lambda(i)} \text{ for } i = 1,\dots, m\}\,.     
    % \]
\end{proof}
