...
\]
is called a generalized orbital variety. 

% 
We need the following result from \cite{dthesis}.
\begin{proposition}
    \label{prop:dth}
    (Part of which is a corollary of \cite[Theorem~4.4.1(c)]{mirkovic2019comparison})
    There is an isomorphism 
    $\phi: \overline{\OO}^\lambda\cap\TT_\mu\cap\n \to \overline\Gr^\lambda\cap S^\mu_-$ 
    and it is compatible with the bijection $YT(\lambda)_\mu \to \irr \overline{\OO}^\lambda\cap\TT_\mu\cap\n$ 
    in the sense that the irreducible component $\phi(X(\tau))$ associated to the semistandard Young tableau $\tau$ is an open dense subset of the MV cycle whose Lusztig datum is equal to that of $\tau$. 
\end{proposition}
...
% 
Now let $m,\lambda',\lambda'',\lambda,\mu',\mu'',\mu,N$ be as before.
% 
...
\acom{Need to introduce LD.}
\begin{proposition}
    Given $b_{Z'},b_{Z''}\in\CC[N]$, let $(\tau',\tau'')\in YT(\lambda')_{\mu'}\times YT(\lambda'')_{\mu''}$ be such that $t^{\lambda'}Z' = X(\tau')$ and $t^{\lambda''}Z'' = X(\tau'')$. Let $\mu = \mu' + \mu''$. If $s\ne 0$, then 
    \[
    \overline{\{A\in\UU_{0,s}^{\mu',\mu''} : A \big|_{\CC^{|\mu(i)|}} \in \OO_{0,s}^{\lambda'(i),\lambda''(i)} \text{ all } i=1,\dots,N\}} 
...
\begin{proof}
    Let $A$ be an element of the set on the left hand side, let $a\in\{0,s\}$ and denote by $E_a$ the generalized $a$-eigenspace of $A$. 
    % Let $i\in \{1,\dots,N\}$ and write $m(i)$ for the maximum weight of a box in $\tau'(i)$ or $\tau''(i)$. 
    
    The the Jordan type of $(A-a)\big|_{E_a\cap\CC^{|\mu(i)|}}$ is given by $\lambda'(i)$ if $a = 0$ and $\lambda''(i)$ if $a = s$. 
    Moreover, the lattice $L$ defined by $g(A)$ is such that $t-a$ has the same Jordan type on 
    % $\cO_a^{m(i)} / L(a)\cap\cO_a^{m(i)} = \CC[t]^{m(i)}/L\cap\CC[t]^{m(i)}\otimes_{\CC[t]}\cO_a $ 
    \begin{equation}
        \label{eq:bcflat}
        \cO_a^{|\mu(i)|} / L(a)\cap\cO_a^{|\mu(i)|} = \left(\CC[t]^{|\mu(i)|}/L\cap\CC[t]^{|\mu(i)|}\right)\otimes_{\CC[t]}\cO_a 
    \end{equation}
    as does $(A-a)\big|_{E_a\cap\CC^{|\mu(i)|}}$. 
    ...
    \Cref{eq:bcflat} follows from the fact that a module over a PID is flat if and only if it is torsion free, and $\cO_a$ is clearly torsion free. 
    ...
    The second assertion is a consequence of \Cref{le:linalg}.
    By \Cref{prop:dth}, it follows that $(L(0),L(s)) \in Z'\times Z''$. 
