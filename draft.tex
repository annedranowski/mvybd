...
\end{definition}    
    ...
    $$
    We obtain the lattice from $ (V,\varphi) $ by setting $ L = \Gamma(D_0, V)$ which is embedded into $ \cK^m = \Gamma(D_0^\times, \Vtriv)$ using $ \varphi$.  On the other hand, we obtain the lattice from the group-theoretic description $ G(\cK) / G(\cO) $ by setting $ L = g \cO^m$ for $ g \in G(\cK)$.\emoji{clap}
    % 
\begin{definition}
 For any $ s \in \CC $, 
...
\end{definition}
    ...
    \begin{definition} The (two point) \textbf{Beilinson--Drinfeld Grassmannian} $\pi : \Grbd\to \AA$ (with one point fixed at 0, and the second point $s\in\AA$ varying).
      \end{definition}
    It is described in modular terms by 
    % {\small
    $$
    ...
    % 
    \begin{definition} The \textbf{positive parts} of $\Gr $ and $\Grth$ are defined by 
    $$
        \Gr^+ = \left(M_m(\cO) \cap G(\cK)\right) / G(\cO) \quad        {\Grth}^+ = \left(M_m(\CC[t]) \cap G(\Kinf)\right) / G(\CC[t])\,. 
    $$
    \end{definition}
...
Now fix effective dominant coweights $\lambda', \lambda'' \in P_{++}$ of sizes $N_1, N_2$  and their sums $ \lambda = \lambda' + \lambda'' $ of size $ N = N_1 + N_2$. 
...
\end{definition}
    ...
    % 
\begin{definition} The \textbf{fusion of two spherical Schubert varieties} $$ \overline{\Gr}^{\lambda', \lambda''}_{0, \AA} = \overline{\Gr}^{\lambda'} \ast_\AA \overline{\Gr}^{\lambda''} \rightarrow \AA $$
\end{definition}
    ...
    Over $s=0$ the fibre is $ \overline{\Gr}^{\lambda' + \lambda''}$ (and reduced) by a theorem of Zhu \cite[Proposition 3.1.14]{zhu2016introduction}. 
