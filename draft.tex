...
% section ??.  
As $ g(A) \in M_n(\CC[t]) \cap G_1(\CC\xt[t^{-1}])$, we will regard $ g(A)$ as giving an element of $ \Grth^+ \cap \cW_\mu$.
...
$$
This is not a very serious abuse of notation, since the map $ \Grbd \rightarrow \Grth $ is almost injective. \acom{Jiuzu was asking about this.}
...
\begin{theorem} \label{th:OGrl}
    ...
    $$\overline{\OO}^{\lambda_1, \lambda_2}_\AA \cap \TT_\mu \cong \overline\Gr^{\lambda_1, \lambda_2}_\AA \cap \cW_\mu $$
    given by $ (A,s) \mapsto (g(A), s)$.
\end{theorem}
\acom{Joel, you stopped writing $X_{0,\AA}$ in favour of $X_\AA$ --- should we do this everywhere?}

\begin{proof}
Since we already have the isomorphism from Theorem \ref{th:TmuWmu}, it suffices to show that for any $ A \in \TT_\mu$, $$ (A,s) \in \overline{\OO}^{\lambda_1, \lambda_2}_\AA \cap \TT_\mu \text{ if and only if } (g(A), s) \in \overline\Gr^{\lambda_1, \lambda_2}_\AA \cap \cW_\mu $$
...
$$
for some $ r \in G(\CC[t]), n \in N[t, t^{-1}, (t-s)^{-1}] $ and $ g = g(A)t^{-\mu}$.  Let $ h = (t-s)^{\mu_2} t^{-\mu_2}$ which lies in $ T_1 \CC[[t]]$.
\acom{sb $T_1\xt[t^{-1}]$ as above?}
Note that $ h^{-1}n h \in N(\Kinf)$, 
% ((t^{-1}))
so we can factor it as $ n = n_1 n_2 $, where $ n_1 \in N_1(\CC\xt[t^{-1}]), n_2 \in N(\CC[t])$.  So then after doing a bit of algebra, we reach
$$
t^\mu r (t^{-\mu} n_2 t^\mu) t^{-\mu} = g^{-1} h n_1.
$$
The right hand side $ g^{-1} g n_1 $ \acom{sb $g^{-1}hn_1$} lies in $ G_1(\CC\xt[t^{-1}]) $.  Now on the left hand side, we know that since $ \mu $ is dominant $ t^{-\mu} n_2 t^\mu \in N(\CC[t])$.  
Moreover since $ \mu $ is dominant, we know that 
$$t^\mu G(\cO) t^{-\mu} \cap G_1(\CC\xt[t^{-1}]) = N_1(\CC\xt[t^{-1}]\,.$$
Thus, we deduce that $ g^{-1} h n_1 \in N_1(\CC\xt[t^{-1}])$ and hence $ g(A) \in N_1(\CC\xt[t^{-1}]) t^{\mu_1} (t-s)^{\mu_2} $.  Given that we know that $ A \in \TT_\mu $, this implies that $ (A,s) \in \UU^{\mu_1, \mu_2}_\AA$ as desired.
\end{proof}
