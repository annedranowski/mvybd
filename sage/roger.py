...
def get_mu(n,t1,t2):
	...
	for i in t2:
		for j in i:
			mu_2[j-1] += 1
	mu = [a+b for a,b in zip(mu_1,mu_2)] # zip creates tuples out of inputs
	return mu_1, mu_2, mu
...
def create_ring(n,t1,t2,mu):
    ...
    # TODO: try with ZZ/big prime
	R.inject_variables() # this creates the variables 
	return R, vari

# insert select rows to a zero matrix 
def insert_r(M,k,row):
	return matrix(M.rows()[:k]+[row]+M.rows()[k:])

# create select nonzero rows 
...
# create weakly upper triangular mu slice block matrix
